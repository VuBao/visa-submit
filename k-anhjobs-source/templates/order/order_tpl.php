		...
		<?php if (isset($_SESSION['cart']) && count($_SESSION['cart'])) { ?>
						...
						<div class="procart procart-<?= $code ?> d-flex align-items-start justify-content-between">
							...
							<div class="info-procart">
								...
								<div class="properties-procart">
									<?php if ($mau) {
										$maudetail = $d->rawQueryOne("select ten$lang as ten from #_product_mau where type = ? and id = ? limit 0,1", array($proinfo['type'], $mau)); ?>
										<p>Màu: <strong><?= $maudetail['ten'] ?></strong></p>
									<?php } ?>
									<?php if ($size) {
										$sizedetail = $d->rawQueryOne("select ten$lang as ten from #_product_size where type = ? and id = ? limit 0,1", array($proinfo['type'], $size)); ?>
			...
			<div class="bottom-cart">
				<div class="section-cart">
					...
					<div class="information-cart">
						<div class="input-double-cart w-clear row custom-row-contact">
							...
							<div class="input-contact custom-col-contact">
								<div class="position-relative custom-hover-input">
									...
									<input type="text" id="ten" name="ten" placeholder="<?= hoten ?>" required value="<?= !empty($_SESSION[$login_member]['ten']) ? $_SESSION[$login_member]['ten'] : '' ?>" />
							...
							<div class="input-contact custom-col-contact">
								<div class="position-relative custom-hover-input">
									...
									<input type="text" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*)\./g, '$1');" id="dienthoai" name="dienthoai" value="<?= !empty($_SESSION[$login_member]['dienthoai']) ? $_SESSION[$login_member]['dienthoai'] : '' ?>" placeholder="<?= sodienthoai ?>" required />
						...
						<div class="row custom-row-contact">
							...
							<div class="input-contact custom-col-contact">
								<div class="position-relative custom-hover-input">
									...
									<input type="email" id="email" name="email" placeholder="Email" value="<?= !empty($_SESSION[$login_member]['email']) ? $_SESSION[$login_member]['email'] : '' ?>" required />
                    ...
                    <div class="position-relative custom-hover-input">
                        ...
                        <input type="text" id="diachi" name="diachi" placeholder="<?= diachi ?>" value="<?= !empty($_SESSION[$login_member]['diachi']) ? $_SESSION[$login_member]['diachi'] : '' ?>" required />
						...
						<div class="row custom-row-contact">
                <div class="input-contact col-sm-12 custom-col-contact">
                    <div class="position-relative custom-hover-input">
                        <textarea id="yeucaukhac" name="yeucaukhac" placeholder="<?= yeucaukhac ?>" /></textarea>
                        <div class="invalid-feedback"><?= yeucaukhac ?></div>
                    </div>
