...
if($id!='')
{
	...
	/* Lấy thương hiệu */
	$pro_brand = $d->rawQueryOne("select ten$lang as ten, tenkhongdauvi, tenkhongdauen, id from #_product_brand where id = ? and type = ? and hienthi > 0",array($row_detail['id_brand'],$type));
	...
	$sanphamdaxem = $d->rawQuery("select ten$lang as ten, tenkhongdauvi, tenkhongdauen, id, photo, gia, giamoi, giakm, id_brand from #_product where type = ? and id in (".$_SESSION['daxem'].") and hienthi > 0 order by stt,id desc limit 0,20",array('san-pham'));
