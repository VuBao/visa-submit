...
class Functions
{
	...
	function giaima_share($encrypted)
	{
		$key = 'secretkey123456';
		$cipher = "aes-256-cbc";
		$iv = 'zsdfubf3)^3n283nl';
	...
	function khoangcach($time)
	{
		$startTime = new DateTime(date('Y-m-d H:i:s',$time));
	...
	public function check_permission()
	{
		...
		$role = isset($_SESSION[$login_admin]['role']) ? (int)$_SESSION[$login_admin]['role'] : 0;

		/*
		 * Tai khoan trong table_user chi co hai cap admin:
		 * - role 3: Super Admin
		 * - role 1: Admin phu
		 *
		 * Ca hai cap deu duoc truy cap cac module nghiep vu. Cac chuc nang
		 * quan ly tai khoan Admin duoc bao ve rieng bang is_super_admin().
		 */
		if(in_array($role, array(1, 3), true) || !empty($config['website']["debug-developer"])) return false;
		return true;
	}

	/* Chỉ Super Admin mới được quản lý tài khoản Admin và nhóm quyền */
	public function is_super_admin()
	{
		global $login_admin;

		return isset($_SESSION[$login_admin]['role']) && (int)$_SESSION[$login_admin]['role'] === 3;
	}
	...
	public function transfer($msg='', $page='', $stt=true)
	{
		...
		if (empty($page_transfer)) {
			$page_transfer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : $config_base;
		}

		if ($stt) {
			$_SESSION['popup_success'] = $msg;
		} else {
			$_SESSION['popup_error'] = $msg;
		}

		$this->redirect($page_transfer);
	}
	...
	{
		$x = (!empty($patch) && file_exists($patch)) ? getimagesize($patch) : null;
		return array(
			"p" => $photo,
			"w" => isset($x[0]) ? $x[0] : 0,
			"h" => isset($x[1]) ? $x[1] : 0,
			"m" => isset($x['mime']) ? $x['mime'] : ''
		);
	}
	...
	/* Get search */
	public function get_search($table='', $type='', $lang='vi', $title_select='Chọn hướng', $class_select='select-category', $select = 0)
	{
		$id_parent = 'id_'.$type;
		...
		foreach($rows as $v)
		{
			if($v['id'] == $select) $selected = "selected";
			else $selected = "";
