...
class PDODb
{
    ...
    public function __construct($type, $host = null, $username = null, $password = null, $dbname = null, $port = null, $charset = null)
    {
        if (is_array($type)) { // if params were passed as array
            ...
            foreach ((array) $this->connectionParams as $key => $param) {
    ...
    private function buildInsert($tableName, $insertData, $operation)
    {
        ...
        $this->createlogs(@$_SESSION[$login_nxn]['username'], 'insert', $tableName, $this->pdo()->lastInsertId());
        $this->reset();
    ...
    public function getOne($tableName, $columns = '*')
    {
        ...
        if ($this->useGenerator) {
            if (is_object($result) && method_exists($result, 'current')) {
    ...
    public function rawQuery($query, $params = null)
    {
        ...
        if (strpos($this->query, 'delete') !== false) {
            ...
            $this->createlogs(@$_SESSION[$login_nxn]['username'], 'query', "tables", $queryLogs);
        }
        $this->reset();
        return $result;
    }
    ...
    {
        $result = $this->rawQuery($query, $params);
        if ($this->useGenerator && !method_exists($result, 'current')) {
            return null;
    ...
    public function update($tableName, $tableData, $numRows = null)
    {
        ...
        $this->createlogs(@$_SESSION[$login_nxn]['username'], 'update', $tableName, $wherelogs);
        $this->reset();
        $this->rowCount      = $stmt->rowCount();
        return $status;
    }
