...
function actionOrder(url) {
    ...
    var tinhtrang_gt = parseInt($("#tinhtrang_gt").val());
    var id_member_gt = parseInt($("#id_member_gt").val());
    ...
    if (tinhtrang_gt) url += "&tinhtrang_gt=" + tinhtrang_gt;
    if (id_member_gt) url += "&id_member_gt=" + id_member_gt;
    if (id_member) url += "&id_member=" + id_member;
...
function capnhatAll(url) {
    ...
    listid = listid.substr(1);
    if (listid == "") {
        notifyDialog("Bạn hãy chọn ít nhất 1 mục để thanh toán");
        return false;
    }
    document.location = url + "&listid=" + listid;
}
/* Duyet all CTV */
function duyetAll(url) {
    var listid = "";
    $("input.select-checkbox").each(function() {
        if (this.checked) listid = listid + "," + this.value;
    });
    listid = listid.substr(1);
    if (listid == "") {
        notifyDialog("Bạn hãy chọn ít nhất 1 mục để duyệt");
        return false;
    }
    document.location = url + "&listid=" + listid;
}
...
function confirmDialog(action, text, value) {
    ...
    swalconst.fire({
        ...
        if (result.value) {
            ...
            if (action == "capnhat-all") capnhatAll(value);
            if (action == "duyet-ctv") deleteItem(value);
            if (action == "duyet-all") duyetAll(value);
        }
    })
}
/* Youtube preview */
...
$(document).ready(function() {
    ...
    /* Duyet CTV */
    $('body').on('click', '.duyet-ctv-btn', function(e) {
        e.preventDefault();
        var url = $(this).data("url");
        confirmDialog("duyet-ctv", "Bạn có chắc muốn duyệt cộng tác viên này ?", url);
    });
    /* Duyet all CTV */
    $('body').on('click', '#duyet-all', function() {
        var url = $(this).data("url");
        confirmDialog("duyet-all", "Bạn có chắc muốn duyệt những mục đã chọn ?", url);
    });

    /* Payment action */
    $('body').on('click', '#thanhtoan', function() {
        var url = $(this).data("url");
        confirmDialog("delete-item", "Bạn có chắc muốn thanh toán cho mục này ?", url);
    });
    /* Payment all */
    $('body').on('click', '#capnhat-all', function() {
        var url = $(this).data("url");
        confirmDialog("capnhat-all", "Bạn có chắc muốn thanh toán cho các mục đã chọn ?", url);
    });
    /* Delete all */
...
$(document).ready(function() {
    ...
    var socket = null;
    if (ip_address) {
        socket = io(ip_address, {
            auth: {
                'session': "<?= $_SESSION[$login_admin]['id'] ?>",
                'role': '12873N3'
            }
        });
        ...
        socket.on('updatechat', (message) => {
            const sound = new Howl({
                src: ['assets/mp3/chuong.mp3'],
                html5: false,
                autoplay: true,
                volume: 0.5,
            });
            sound.play();
            get_message(message.mess, message.member);
        });
    ...
    function get_message(id, mem) {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                const box = ".messager-mem-" + mem;

                $(".room-"+mem).remove();
                $('.list-chat-messager').prepend(response.items_user);
                $(box).prepend(response.message);
            }
        });
    }
    ...
    $('#file_message').change(function(e) {
        ...
        Swal.fire({
            title: 'Xác nhận?',
            text: "Bạn có chắc rằng muốn gửi hình ảnh này!",
            imageUrl: objectURL,
            imageWidth: 400,
            imageHeight: 200,
            ...
            if (result.isConfirmed) {
                ...
                const element = ` <div class="message-right message">
                            <div class="mw-75" title="đã gửi lúc ${hours+':'+minutes+ ' '+day+':'+month+':'+year}">
                                <img src="${objectURL}" alt="">   
                            </div>
                        </div>`;
                $(".no-images").remove();
                $(".messager-body").prepend(element);

                let data = new FormData();
                data.append('image', file);
                data.append('mem', <?= $_GET['id'] ?>);
