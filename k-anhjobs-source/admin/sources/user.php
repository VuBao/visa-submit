...
function save_item_admin()
{
	...
	if($username === '') $func->transfer("Chưa nhập tên đăng nhập", "index.php?com=user&act=man_admin&p=".$curPage, false);

	if($id)
	{
		$row = $d->rawQueryOne("select id, username from #_user where id = ? and role = 1 limit 0,1",array($id));
		if(empty($row['id'])) $func->transfer("Tài khoản Admin phụ không tồn tại", "index.php?com=user&act=man_admin&p=".$curPage, false);
		if(strcasecmp($username, 'admin') === 0) $func->transfer("Tên đăng nhập admin được dành riêng cho Super Admin", "index.php?com=user&act=edit_admin&id=".$id."&p=".$curPage, false);
		$duplicate = $d->rawQueryOne("select id from #_user where username = ? and id <> ? limit 0,1",array($username,$id));
		if(isset($duplicate['id']) && $duplicate['id'] > 0) $func->transfer("Tên đăng nhập này đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=edit_admin&id=".$id."&p=".$curPage, false);
	...
	{
		if(strcasecmp($username, 'admin') === 0) $func->transfer("Tên đăng nhập admin được dành riêng cho Super Admin", "index.php?com=user&act=add_admin&p=".$curPage, false);
		$row = $d->rawQueryOne("select id from #_user where username = ? limit 0,1",array($username));
		if(isset($row['id']) && $row['id'] > 0) $func->transfer("Tên đăng nhập này đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=add_admin&p=".$curPage, false);

		if(isset($data['password']) && ($data['password'] == '')) $func->transfer("Chưa nhập mật khẩu", "index.php?com=user&act=add_admin&p=".$curPage, false);
		$confirm_password = isset($_POST['confirm_password']) ? $_POST['confirm_password'] : '';
		if($confirm_password === '' || $data['password'] !== $confirm_password) $func->transfer("Xác nhận mật khẩu không chính xác", "index.php?com=user&act=add_admin&p=".$curPage, false);
...
function get_items()
{
	...
	$startpoint = ($curPage * $per_page) - $per_page;
	$limit = " limit ".$startpoint.",".$per_page;
	$sql = "select * from #_member where id <> 0 $where and role = 0 order by stt,id desc $limit";
	$items = $d->rawQuery($sql);
	$sqlNum = "select count(*) as 'num' from #_member where id <> 0 $where and role = 0 order by stt,id desc";
	$count = $d->rawQueryOne($sqlNum);
	$total = $count['num'];
...
function save_item()
{
	...
	if($email !== '') {
		...
		if(!empty($check_email)) $func->transfer("Email này đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=man&p=".$curPage, false);
	}
...
function delete_item()
{
	...
	if($id)
	{
		...
		$d->rawQuery("delete from #_ungtuyen where id_member = ?",array($id));
...
/* Get ctv */
function get_items_ctv()
{
	global $d, $func, $curPage, $items, $paging, $config, $allCTV, $allChoDuyet, $allTodayLogin;
	...
	$startpoint = ($curPage * $per_page) - $per_page;
	$limit = " limit ".$startpoint.",".$per_page;
	$sql = "select * from #_member where id <> 0 $where and role = 1 and tinhtrang = 1 order by stt,id desc $limit";
	$items = $d->rawQuery($sql);
	$sqlNum = "select count(*) as 'num' from #_member where id <> 0 $where and role = 1 and tinhtrang = 1 order by stt,id desc";
	$count = $d->rawQueryOne($sqlNum);
	$total = $count['num'];
	$url = "index.php?com=user&act=man_ctv";
	$paging = $func->pagination($total,$per_page,$curPage,$url);

	$allCTV_query = $d->rawQueryOne("select count(id) as count from #_member where role = 1 and tinhtrang = 1");
	$allCTV = $allCTV_query['count'];

	$allChoDuyet_query = $d->rawQueryOne("select count(id) as count from #_member where role = 1 and tinhtrang = 0");
	$allChoDuyet = $allChoDuyet_query['count'];

	$today_start = strtotime(date("Y-m-d 00:00:00"));
	$allTodayLogin_query = $d->rawQueryOne("select count(id) as count from #_member where role = 1 and tinhtrang = 1 and lastlogin >= ?", array($today_start));
	$allTodayLogin = $allTodayLogin_query['count'];
}

/* Edit visitor */
function get_item_ctv()
{
	global $d, $func, $curPage, $item;

	$id = (isset($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

	if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=user&act=man_ctv&p=".$curPage, false);

	$item = $d->rawQueryOne("select * from #_member where id = ? limit 0,1",array($id));

	if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=user&act=man_ctv&p=".$curPage, false);
}

/* Save visitor */
function save_item_ctv()
{
	global $d, $func, $curPage;

	if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=user&act=man_ctv&p=".$curPage, false);
	...
	if($username !== '') {
		$check_username = $d->rawQueryOne("select id from #_member where username = ? and id <> ? limit 0,1",array($username, $id));
		if(!empty($check_username)) $func->transfer("Tên đăng nhập này đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=man_ctv&p=".$curPage, false);
	}

	$email = isset($data['email']) ? $data['email'] : '';
	if($email !== '') {
		$check_email = $d->rawQueryOne("select id from #_member where email = ? and id <> ? limit 0,1",array($email, $id));
		if(!empty($check_email)) $func->transfer("Email này đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=man_ctv&p=".$curPage, false);
	}
	...
	if($id)
	{
		...
		{
			$row = $d->rawQueryOne("select id from #_member where id = ? limit 0,1",array($id));
			if(isset($row['id']) && $row['id'] > 0) $func->transfer("Bạn không có quyền trên tài khoản này. Mọi thắc mắc xin liên hệ quản trị website", "index.php?com=user&act=man_ctv&p=".$curPage, false);
		}
		...
		if(isset($data['password']) && ($data['password'] != ''))
		{
			...
			if($confirm_password == '')
			{
				$func->transfer("Chưa xác nhận mật khẩu mới","index.php?com=user&act=edit_ctv&id=".$id."&p=".$curPage, false);
			}

			if($password != $confirm_password)
			{
				$func->transfer("Xác nhận mật khẩu mới không chính xác","index.php?com=user&act=edit_ctv&id=".$id."&p=".$curPage, false);
			}
		...
		$d->where('id', $id);
		if($d->update('member',$data)) $func->transfer("Cập nhật dữ liệu thành công", "index.php?com=user&act=man_ctv&p=".$curPage);
		else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=user&act=man_ctv&p=".$curPage, false);
	}
	else
	{
		// $username = isset($data['username']) ? $data['username'] : '';
		...
		if(isset($data['password']) && ($data['password'] == '')) $func->transfer("Chưa nhập mật khẩu", "index.php?com=user&act=add_ctv&p=".$curPage, false);
		$data['password'] = md5($data['password']);

		if($d->insert('member',$data)) $func->transfer("Lưu dữ liệu thành công", "index.php?com=user&act=man_ctv&p=".$curPage);
		else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=user&act=man_ctv&p=".$curPage, false);
	}
}

/* Delete visitor */
function delete_item_ctv()
{
	global $d, $func, $curPage;
	...
	{
		$d->rawQuery("delete from #_member where id = ?",array($id));
		$d->rawQuery("delete from #_ungtuyen where id_member = ? or id_member_gt = ?",array($id, $id));
		$func->transfer("Xóa dữ liệu thành công", "index.php?com=user&act=man_ctv&p=".$curPage);
	}
	elseif(isset($_GET['listid']))
	{
		$listid = explode(",",$_GET['listid']);
	...
	$func->transfer("Không nhận được dữ liệu","index.php?com=user&act=man_ctv&p=".$curPage, false);
}
...
function edit()
{
	...
	$current_user_id = isset($_SESSION[$login_admin]['id']) ? (int)$_SESSION[$login_admin]['id'] : 0;
	...
	if(!empty($_POST))
	{
		...
		if(isset($changepass) && $changepass == 1)
		{
			...
			if($old_pass!='' || $new_pass!='' ||  $renew_pass!='')
			{
				...
				/* Lấy dữ liệu */
				$row = $d->rawQueryOne("select id, password from #_user where id = ? limit 0,1",array($current_user_id));
		...
		{
			$username = isset($data['username']) ? trim($data['username']) : '';
			if($func->is_super_admin()) $username = $data['username'] = 'admin';
			elseif(strcasecmp($username, 'admin') === 0) $func->transfer("Tên đăng nhập admin được dành riêng cho Super Admin","index.php?com=user&act=admin_edit", false);
			$row = $d->rawQueryOne("select id from #_user where id <> ? and username = ? limit 0,1",array($current_user_id,$username));
			if(isset($row['id']) && $row['id'] > 0) $func->transfer("Tên đăng nhập này đã tồn tại","index.php?com=user&act=admin_edit", false);

			$data['ngaysinh'] = strtotime(str_replace("/","-",$data['ngaysinh']));
		}

		$d->where('id', $current_user_id);
		if($d->update('user',$data))
		{
			if(isset($data['username']) && $data['username'] !== '') $_SESSION[$login_admin]['username'] = $data['username'];
	...
	$item = $d->rawQueryOne("select * from #_user where id = ? limit 0,1",array($current_user_id));
}
