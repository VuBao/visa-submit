...
if($error == '')
{
	...
	{
		/* Kiểm tra đăng nhập */
		/* Username admin luôn ưu tiên record Super Admin (role 3). */
		$row = $d->rawQueryOne("select * from #_user where username = ? and hienthi>0 order by role desc, id asc limit 0,1",array($username));
