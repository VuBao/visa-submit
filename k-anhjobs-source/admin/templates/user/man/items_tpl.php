    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">Quản lý tài khoản ứng viên</li>
            </ol>
        </div>
    </div>
</section>
...
<section class="content">
    ...
    <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header">
            <h3 class="card-title">Danh sách tài khoản ứng viên</h3>
        </div>
                            ...
                            <tr>
                                ...
                                <td class="align-middle">
                                    ...
                                    if (!empty($items[$i]['id_member_gt'])) {
                                        $gt_mem = $d->rawQueryOne("select mactv from #_member where id = ? limit 0,1", array($items[$i]['id_member_gt']));
                                        if (!empty($gt_mem['mactv'])) {
                                            $mactv_prefix = "[" . $gt_mem['mactv'] . "] ";
                                        }
                                    }
                                    ?>
                                    <a class="text-dark" href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['ten']?>"><?= $mactv_prefix ?><?= $items[$i]['ten'] ? $items[$i]['ten'] : $items[$i]['username'] ?></a>
                                </td>
                                ...
                                <td class="align-middle">
                                    <?php if($items[$i]['link_facebook']) { ?> 
                                        <a href="<?=$items[$i]['link_facebook']?>" title="<?=$items[$i]['username']?>">Truy cập ngay</a>
                                    <?php } else { ?> 
                                        Chưa cập nhật
                                    <?php } ?>
                                </td>
                                ...
                                <td class="align-middle text-center text-md text-nowrap">
                                    <a class="text-success mr-2" href="<?=$linkChat?>&id=<?=$items[$i]['id']?>" title="Chat"><i class="far fa-comment-dots"></i></a>
